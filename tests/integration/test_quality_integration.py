...
class TestQualityIntegration(BaseIntegrationTest):
    ...
    def test_artifact_detection_and_refinement(
        self, expandor, mock_inpaint_pipeline, temp_dir
    ):
        ...
        assert (
            "final_quality_score" in result.metadata
            or "quality_refined" in result.metadata
        )
    ...
    def test_boundary_tracking_integration(
        self, expandor, mock_inpaint_pipeline, temp_dir
    ):
        ...
        assert (
            "boundaries" in result.metadata or "boundary_positions" in result.metadata
        )

        # If quality was validated, boundaries should have been analyzed
        if "final_quality_score" in result.metadata:
            print(
                f"Quality validated with {len(result.metadata.get('boundaries', []))} boundaries"
            )
