...
class TestExpandorIntegration:
    ...
    def test_vram_aware_strategy_selection(self):
        ...
        with patch.object(
            self.expandor.vram_manager, "get_available_vram"
        ) as mock_vram:
            mock_vram.return_value = 2000  # 2GB

            with patch.object(
                self.expandor.vram_manager, "calculate_generation_vram"
            ) as mock_calc:
    ...
    def test_pipeline_fallback_chain(self):
        ...
        with patch(
            "expandor.strategies.direct_upscale.DirectUpscaleStrategy.execute"
        ) as mock_exec:
            mock_exec.side_effect = VRAMError(
                operation="upscale", required_mb=20000, available_mb=8000
            )
