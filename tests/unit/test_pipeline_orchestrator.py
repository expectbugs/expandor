...
class TestPipelineOrchestrator:
    ...
    def setup_method(self):
        """Setup for each test"""
        self.config = {
            "vram_strategies": {"fallback_chain": {1: "tiled_large", 2: "cpu_offload"}}
        }
    ...
    def test_strategy_preparation(self):
        ...
        # Register pipelines in orchestrator
        inpaint_pipeline = Mock()
        refiner_pipeline = Mock()
        self.orchestrator.register_pipeline("inpaint", inpaint_pipeline)
        self.orchestrator.register_pipeline("refiner", refiner_pipeline)

        self.orchestrator._prepare_strategy(
            strategy, config, self.boundary_tracker, self.metadata_tracker
        )

        assert strategy.inpaint_pipeline == inpaint_pipeline
        assert strategy.refiner_pipeline == refiner_pipeline
    ...
    def test_vram_error_handling(self):
        ...
        primary_strategy = MockStrategy()
        primary_strategy.should_fail = True
        primary_strategy.execute = Mock(
            side_effect=VRAMError(operation="test", required_mb=8000, available_mb=4000)
        )
