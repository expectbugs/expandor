...
class TestMetadataTracker:
    ...
    def test_start_operation(self):
        ...
        config.get_target_resolution = Mock(return_value=(1920, 1080))
        config.quality_preset = "balanced"
        config.prompt = "Test prompt"
        config.negative_prompt = None
        config.seed = 42
        config.strategy = "auto"
        config.strategy_override = None
        config.denoising_strength = 0.95
    ...
    def test_add_stage(self):
        ...
        stage_event = next(
            e for e in self.tracker.events if e["type"] == "stage_metadata"
        )
