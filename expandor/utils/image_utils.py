...
def create_gradient_mask(width: int, height: int, direction: str, 
                        blur_radius: int, fade_start: float = 0.0) -> Image.Image:
    """
...
def blend_images(img1: Image.Image, img2: Image.Image, 
                mask: Image.Image, mode: str = 'normal') -> Image.Image:
    """
    ...
    if img1.size != img2.size or img1.size != mask.size:
        logger.warning(f"Image sizes don't match: {img1.size}, {img2.size}, {mask.size}")
...
def extract_edge_colors(image: Image.Image, edge: str, 
                       width: int = 1, method: str = 'average') -> np.ndarray:
    """
...
def create_noise_pattern(width: int, height: int, scale: float = 0.1,
                        octaves: int = 1, seed: Optional[int] = None) -> np.ndarray:
    """
...
def create_circular_mask(size: Tuple[int, int], center: Tuple[int, int],
