...
class GPUMemoryManager:
    ...
    def estimate_tensor_memory(self, shape: Tuple[int, ...], 
    ...
    def has_sufficient_memory(self, required_mb: float, 
    ...
    @contextmanager
    def memory_efficient_scope(self, name: str = "operation",
    ...
    def get_optimal_batch_size(self, base_memory_mb: float,
...
def offload_to_cpu(tensor: torch.Tensor, name: str) -> torch.Tensor:
    ...
    if tensor.is_cuda:
        ...
        logger.debug(f"Offloaded {name} to CPU ({tensor.element_size() * tensor.nelement() / 1024**2:.1f}MB)")
        return cpu_tensor
    return tensor


def load_to_gpu(tensor: torch.Tensor, name: str, 
    ...
    if not tensor.is_cuda:
        ...
        logger.debug(f"Loaded {name} to GPU ({tensor.element_size() * tensor.nelement() / 1024**2:.1f}MB)")
...
def estimate_model_memory(model: Any, include_gradients: bool = True) -> float:
    ...
    logger.debug(f"Model memory estimate: {total_mb:.1f}MB ({total_params:,} parameters)")
