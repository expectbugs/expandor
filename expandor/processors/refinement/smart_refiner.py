...
class SmartRefiner:
    ...
    def refine_image(self,
                    ...
                    stage_dir: Optional[Path] = None) -> RefinementResult:
        """
        ...
        self.logger.info(f"Created {len(regions)} refinement regions from {len(artifacts)} artifacts")
        ...
        for iteration in range(self.max_iterations):
                ...
                try:
                    ...
                    if refined is not None:
                        ...
                        if save_stages and stage_dir:
                            stage_path = stage_dir / f"refine_iter{iteration}_region{regions.index(region)}.png"
            ...
            self.logger.info(f"Quality after iteration {iteration + 1}: {current_quality:.3f}")
            
            if current_quality >= self.quality_threshold:
                self.logger.info(f"Quality threshold reached: {current_quality:.3f} >= {self.quality_threshold}")
                break
            
            # Update regions for next iteration based on remaining issues
            if quality_check['has_issues']:
                new_artifacts = quality_check['seam_artifacts'] + quality_check['general_artifacts']
                if new_artifacts:
                    regions = self._create_refinement_regions(new_artifacts, current_image.size)
                else:
                    break
            else:
                break
        ...
        original_quality = self.edge_analyzer.analyze_image(image, boundaries)['quality_score']
        final_quality = self.edge_analyzer.analyze_image(current_image, boundaries)['quality_score']
    ...
    def _create_refinement_regions(self, artifacts: List[EdgeInfo],
                                 image_size: Tuple[int, int]) -> List[RefinementRegion]:
        """
        Group artifacts into refinement regions.
        ...
        for i, artifact in enumerate(artifacts):
            ...
            if artifact_type == 'seam':
                strength = min(0.9, self.base_strength * self.strength_multiplier * severity)
                blur = 32
    ...
    def _refine_region(self, image: Image.Image, region: RefinementRegion,
                      pipeline: Any, prompt: str) -> Optional[Image.Image]:
        """
        ...
        if region.artifact_type == 'seam':
            context_prompt = f"{prompt}, seamless blending, smooth transition, no visible seams"
        elif region.artifact_type == 'pattern':
            context_prompt = f"{prompt}, natural texture, varied patterns, no repetition"
        else:
    ...
    def _create_refinement_map(self, regions: List[RefinementRegion],
    ...
    def create_quality_report(self, original: Image.Image, refined: Image.Image,
        ...
        quality_improvement = refined_analysis['quality_score'] - original_analysis['quality_score']
        artifacts_removed = len(original_analysis['seam_artifacts']) - len(refined_analysis['seam_artifacts'])
        
        report = {
            'original_quality': original_analysis['quality_score'],
            'refined_quality': refined_analysis['quality_score'],
            'quality_improvement': quality_improvement,
            'improvement_percentage': (quality_improvement / original_analysis['quality_score']) * 100,
            'original_artifacts': {
