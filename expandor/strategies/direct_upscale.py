...
class DirectUpscaleStrategy(BaseExpansionStrategy):
    ...
    def validate_requirements(self):
        ...
        if not self.realesrgan_path:
            raise StrategyError(
                "Real-ESRGAN not found. DirectUpscaleStrategy requires Real-ESRGAN.\n"
                "Solutions:\n"
    ...
    def _find_realesrgan(self) -> Optional[Path]:
        ...
        try:
            result = subprocess.run(
                ["which", "realesrgan-ncnn-vulkan"], capture_output=True, text=True
            )
    ...
    def execute(
        ...
        if passes_needed > 1:
            self.logger.info(
                f"Large upscale factor {
                    scale:.1f}x requires {passes_needed} passes"
            )
    ...
    def _run_realesrgan(
        ...
        if not self.realesrgan_path:
            ...
            return self._run_pil_upscale(input_path, output_path, scale)
        ...
        try:
            ...
            if e.stderr:
                error_msg += f"\nError: {e.stderr}"
            raise UpscalerError(
                error_msg, tool_name="realesrgan", exit_code=e.returncode
            )
