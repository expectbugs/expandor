...
class ControlNetProgressiveStrategy(ProgressiveOutpaintStrategy):
    ...
    # VRAM priority set in config

    def __init__(self, config: ExpandorConfig,
        ...
        # Get strategy config
        try:
            self.strategy_config = self.config_manager.get_strategy_config('controlnet_progressive')
        except ValueError as e:
            # FAIL LOUD
            raise ValueError(
                f"Failed to load controlnet_progressive configuration!\n{str(e)}"
            )
        
        # Set VRAM priority from config
        if hasattr(self.__class__, 'VRAM_PRIORITY'):
            self.__class__.VRAM_PRIORITY = self.strategy_config['vram_priority']
        
        # Load ControlNet configuration - this is kept separate as it's user-provided
        try:
            controlnet_config = self.config_manager.get_value('controlnet_config')
        except ValueError:
            controlnet_config = {}
        ...
        self.controlnet_strength = self.controlnet_config.get(
            "controlnet_strength")
        if self.controlnet_strength is None:
            # Try strategy config
            self.controlnet_strength = self.strategy_config.get('conditioning_scale')
            if self.controlnet_strength is None:
                raise ValueError(
                    "controlnet_strength not found in controlnet_config or strategy config.\n"
                    "Provide it in controlnet_config or check comprehensive_defaults.yaml"
                )

        # Get extract_at_each_step from controlnet_config or use default from strategy config
        self.extract_at_each_step = self.controlnet_config.get(
            "extract_at_each_step",
            True  # Default to True for ControlNet
        )
        ...
        control_types_requiring_extraction = self.strategy_config['control_types_requiring_extraction']

        if self.control_type in control_types_requiring_extraction:
            try:
                from ..processors.controlnet_extractors import \
                    ControlNetExtractor
    ...
    def _prepare_control_image(
        ...
        if self.control_type == "canny":
            if not self.extractor:
                raise RuntimeError(
                    f"Extractor not initialized for control type '{
            ...
            # For other types (user provided control), expect it in
            # controlnet_config
            ...
            if not isinstance(control_image, Image.Image):
                raise TypeError(
                    f"control_image must be a PIL Image, got {
                        type(control_image)}")
    ...
    def _expand_step(
        ...
        **kwargs
        ...
        canvas = Image.new("RGB", target_size, color=self.strategy_config['canvas_background_color'])
        x_offset = (width - current_width) // self.strategy_config['canvas_centering_divisor']
        y_offset = (height - current_height) // self.strategy_config['canvas_centering_divisor']
        canvas.paste(current_image, (x_offset, y_offset))

        # Create mask for new content
        mask = Image.new("L", target_size, color=self.strategy_config['mask_inpaint_value'])  # White = inpaint
        mask_draw = ImageDraw.Draw(mask)
        mask_draw.rectangle(
            [x_offset, y_offset, x_offset + current_width -
                1, y_offset + current_height - 1],
            fill=self.strategy_config['mask_keep_value']  # Black = keep
        )

        # Blur mask for smooth transitions
        blur_radius = self._calculate_blur_radius(
            target_size, current_image.size)
        ...
        base_strength = kwargs.get(
            "strength", 
            self.strategy_config.get('default_strength', self.strategy_config['strength_fallback'])
        )
    ...
    def validate_support(self) -> bool:
        ...
        if not self.adapter.supports_controlnet():
            raise RuntimeError(
                f"Adapter does not support ControlNet for model type: {
                    getattr(
                        self.adapter,
                        'model_type',
