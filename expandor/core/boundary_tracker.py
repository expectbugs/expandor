...
class BoundaryTracker:
    ...
    def reset(self):
        ...
        self.progressive_boundaries: List[Dict] = []  # For checklist compatibility
    ...
    def add_progressive_boundary(self, position: int, direction: str, step: int, 
        ...
        if expansion_size is not None:
            self.add_boundary(
                ...
                step=step,
                expansion_size=expansion_size,
                source_size=(0, 0),  # Will be overridden by add_progressive_boundaries
                target_size=(0, 0),
                method='progressive',
                metadata=metadata
            )
    ...
    def get_boundary_regions(self, width: int, height: int, padding: int = 10) -> List[Tuple[int, int, int, int]]:
        """
