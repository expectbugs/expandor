...
class PipelineOrchestrator:
    ...
    def _prepare_strategy(
        ...
        """
        # Inject pipelines from registry only
        if "inpaint" in self.pipeline_registry:
            strategy.inpaint_pipeline = self.pipeline_registry["inpaint"]

        if "refiner" in self.pipeline_registry:
            strategy.refiner_pipeline = self.pipeline_registry["refiner"]

        if "img2img" in self.pipeline_registry:
    ...
    def _execute_single_strategy(
        ...
        # This context is passed to strategies to provide access to shared
        # resources
        context = {
            ...
            "stage_callback": lambda stage: self._record_stage(
                stage, metadata_tracker
            ),  # Stage recording function
            "save_stages": getattr(
    ...
    def _build_fallback_chain(
        self, primary_strategy: BaseExpansionStrategy, config
    ) -> List[BaseExpansionStrategy]:
        """
        ...
        # Get strategy name
        primary_name = primary_strategy.__class__.__name__.replace(
            "Strategy", ""
        ).lower()
        ...
        for priority in sorted(self.fallback_config.keys()):
            ...
            if fallback_name in fallback_map:
                ...
                # Skip if disabled
                if strategy_name == "tiled_expansion" and not getattr(
                    config, "allow_tiled", True
                ):
                    continue
                if strategy_name == "cpu_offload" and not getattr(
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Could not create fallback {strategy_name}: {e}"
                    )
    ...
    def _convert_to_result(
        self, raw_result: Dict[str, Any], strategy: BaseExpansionStrategy, config
    ) -> ExpandorResult:
